...
export default function KnowledgeSphere() {
  ...
  useEffect(() => {
    ...
    const SPHERE_RADIUS = Math.min(width, height) * 0.50;
    const MAX_CONNECTION_DIST = SPHERE_RADIUS * 0.25;
    const PRIMARY_COLOR = '#FF7A18';
    const GLOW_COLOR = 'rgba(255,122,24,0.4)';

    const nodes = [];
    const baseLabels = ["Frontier Models", "Open Source", "Agents", "Fine-tuning", "RAG Systems", "Vector DBs", "Evaluation", "Papers", "Startups", "Frameworks", "Transformers", "Multimodal", "Hugging Face", "GitHub Trending", "AI Tools"];
      ...
      for (let i = 0; i < sortedNodes.length; i++) {
        ...
        for (let j = i + 1; j < sortedNodes.length; j++) {
          ...
          if (dist3D < MAX_CONNECTION_DIST) {
            ...
            const baseColorRGB = isDark ? '148, 163, 184' : '51, 65, 85';

            let n1Color = `rgba(${baseColorRGB}, ${opacity})`;
            let n2Color = `rgba(${baseColorRGB}, ${opacity})`;
            ...
            if (isHovered && dist3D < MAX_CONNECTION_DIST * 1.2) {
              ...
              n2Color = `rgba(${baseColorRGB}, ${opacity})`; // fade out to base color
              lineWidth = 1.2;
              ...
              if (isN1Pulsing && isN2Pulsing) {
                ...
                n1Color = `rgba(255, 122, 24, ${finalOpacity})`;
                n2Color = `rgba(${baseColorRGB}, ${opacity})`;
              } else {
                n1Color = `rgba(${baseColorRGB}, ${opacity})`;
                n2Color = `rgba(255, 122, 24, ${finalOpacity})`;
              }
              lineWidth = 1.0;
            }
        ...
        // Draw Node
        const drawRadius = (n1.baseRadius) * (n1.scale * 2.0); // Base size
        ...
        if (n1.isPulsing) {
          ...
          if (n1.label) {
            ctx.fillStyle = isDark ? '#ffffff' : '#0f172a'; // White or Slate 900 for label anchor nodes
          } else {
            const baseNodeRGB = isDark ? '148, 163, 184' : '51, 65, 85';
            ctx.fillStyle = `rgba(${baseNodeRGB}, ${isDark ? Math.min(1, depthAlpha + 0.2) : depthAlpha})`;
          }
          ctx.fill();
        }
        ...
        if (n1.label && n1.z < 20) { // Only show labels for front and middle nodes
          ...
          if (isDark) {
            ctx.fillStyle = `rgba(248, 250, 252, ${Math.min(1, depthAlpha + 0.6)})`; // Bright text for dark mode
          } else {
  ...
  return (
    <div style={styles.container}>
      {/* Background Grid */}
      <div style={styles.gridOverlay} className="sphere-grid-overlay" />
      ...
      <div style={styles.glassCard}>
        <div style={styles.cardTopRow}>
          ...
          <div style={styles.cardHeader}>LIVE PIPELINE</div>
        </div>
        <div style={styles.cardValue}>Monitoring 126 sources</div>
      </div>
    </div>
  );
}
...
const styles = {
  ...
  cardHeader: {
    color: 'var(--textMuted)', // Adapts to dark/light
  ...
  cardValue: {
    color: 'var(--textMain)', // Adapts to dark/light
    fontSize: '12px',
    fontWeight: '600',
